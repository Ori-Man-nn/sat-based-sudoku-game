...
int sudoku_table[9][9];//定义全局变量int类型二维数组存储数独
...
status Sudoku(void) {
    ...
    while (op) {
        ...
        printf("\t\t                   #请选择功能#\n");
        printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
        printf("   1.数独游戏        2.进阶对角线数独游戏           0.退出游戏\n");
        printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
        scanf("%d", &op);
        switch (op) {
        case 1:
            ...
            break;
        case 2:
            solut = CreateXSudoku();//生成基础数独终盘对应变元真值表
            if (solut) {
                SudokuComplete();
                CNFSudokuTableTransform();//将求解的变元真值表转换为二维数组数独终盘
                SudokuExchange();//生成交换元素增加随机性后的数独终盘
...
/*创建对角线数独问题转化为SAT问题后的cnf文件*/
FILE* CreateXSudokuFile(void) {
    int x, y, z, i, j, k, l;//x代表数独的行，y代表数独的列，z取1～9分别代表该格中数独填入值为1～9中任一值
    FILE* fp;
    fp = fopen("SudokuTableBase.cnf", "w");
    if (fp == NULL) {
